<?php
/*
Plugin Name: Fork from WP Show Posts
Plugin URI: https://www.chizzini.com
Description: WP Show Posts allows you to list posts (from any post type) anywhere on your site. T -  <strong>MOD by MC DO NOT UPDATE.</strong>
Version: 0.1.0
Author: Marco Chizzini
Author URI: https://chizzini.com
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Text Domain: fork-wp-show-posts
*/

/* MOD:
 * added action 'wpspx_after_article' $i, $settings
 */
...
function wpsp_display($id, $custom_settings = false)
{
	...
	$settings['wrapper_class'][] = 'wp-list-' . clean(get_the_title($settings['list_id']));
	...
	if(is_array($args['post__not_in']) && is_front_page()){
		$args['post__not_in'] = array_merge($excludeIDDouble,$args['post__not_in']);
	} else {
		$args['post__not_in'] = $excludeIDDouble;
	}
	...
	if (get_the_title($settings['list_id']) != 'EditorialeRotation') {
		$excludeFlagCat= array('no-homepage','editoriale');
	} else {
		$excludeFlagCat= array('no-homepage');
	}
	$args['tax_query'][] =   array('taxonomy' => 'post_flag', 'field'    => 'slug', 'terms'    => $excludeFlagCat, 'operator' => 'NOT IN');
	...
	if ($count < $args['posts_per_page']) {
		$argsN = array(
			'posts_per_page' => ($args['posts_per_page'] - $count),
			'post__not_in' => wp_list_pluck($query->posts, 'ID')
		);
		$argsN['tax_query'] = array(
			array(
				'taxonomy' => 'post_flag',
				'field'    => 'slug',
				'terms'    => $excludeFlagCat,
				'operator' => 'NOT IN'
			),
		);
		...
		$newquery = new WP_Query($argsN);

		$query->posts =  array_merge($query->posts, $newquery->posts);
		//                $query = new WP_Query(array('post__in' => wp_list_pluck($query->posts, 'ID'),'orderby' => 'post__in'));
		$query = new WP_Query(
			array(
	...
	if (isset($custom_settings['array_id']) && $custom_settings['array_id'] == true) {
	...
	if ($query->have_posts()) {
		...
		while ($query->have_posts()) {
			++$i;
			if ($i >  $args['posts_per_page']) {
				break;
			}
			$query->the_post();

			if (get_the_title($settings['list_id']) != 'LastNews') {
				$excludeIDDouble[] = get_the_ID();
			}			
			...
			printf(
				'<%1$s class="%2$s" itemtype="http://schema.org/%3$s" itemscope>',
			...
			if ($yoast_meta) { 
				...
				<?php
			}elseif ('excerpt' == $settings['content_type'] && $settings['excerpt_length'] && !$more_tag && 'none' !== $settings['content_type']) { ?>
				<div class="wp-show-posts-entry-summary" itemprop="text">
...
<?php }
			...
			do_action('wpspx_after_article', isset($counter) ? ++$counter : ($counter = 0), $settings); // MODbyFB
		}
	} else {
		// no posts found
